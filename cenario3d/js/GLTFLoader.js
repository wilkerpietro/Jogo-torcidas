/* =========================================================
   GLTFLoader + SkeletonUtils — os dois avulsos do three.js
   ---------------------------------------------------------
   Vieram do pacote do artefato "Vitrine dos Bonecos" (three
   r147, versão de script solto) porque é de lá que vem o
   boneco do Blender, e este ambiente não alcança CDN nenhuma.
   Duas adaptações, e só duas:

   1. ELES ESCREVEM EM `THREE`, E O `THREE` DE MÓDULO É SELADO.
      O avulso termina com `THREE.GLTFLoader = GLTFLoader`, que
      num módulo ES (sempre em modo estrito) estoura sobre o
      namespace importado. A saída é uma CÓPIA mutável do
      namespace: as classes são as mesmas, então `instanceof`
      e tudo o mais continuam valendo.

   2. `encoding` VIROU `colorSpace` no r152. O avulso é do
      r147 e o vendido aqui é o r160: `THREE.sRGBEncoding` não
      existe mais e `texture.encoding` não faz nada. As cinco
      chamadas passam `SRGBColorSpace` e o parser escreve em
      `colorSpace` — sem isso a textura do rosto sai lavada.
   ========================================================= */
import * as NS from 'https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.module.min.js';

/* a cópia mutável: é nela que os dois avulsos se penduram */
...
export const SkeletonUtils = THREE.SkeletonUtils;
