...
**A relação sobe +50** com quem ganhou, seja qual for o presente (régua do dono, 17/09/2026 — a primeira versão subia um ponto a cada R$ 10 mil, entre 3 e 15, e durou uma publicação). O presente entra nas nossas transações ("Presente pra Bamor: loja nova") e no extrato dela ("Presente da Leões da TUF: loja nova"), sem mexer no caixa dela.
...
## O veredicto da campanha espera a bola rolar (correção do dono, 21/09/2026)

**Problema.** "Caímos na terceira fase da Copa do Brasil pro Corinthians… Vamos pra porta do CT?" chegava no feed com a partida ainda marcada pra 21h30 no plano da semana. O dia é simulado na virada (`estado.js` → `competicoes.jogarDia`) e o passo "veredicto da campanha" do feed roda logo depois, no mesmo instante: a chave já sabia quem caiu de manhã.

**Decisão.** `desfechoDaCompeticao` (feed.js) devolve `null` enquanto houver jogo NOSSO naquela competição hoje (`jogosDaSemana(...).some(dia === hoje && compId === comp.id)`). `veredictosVistos` só é carimbado quando há desfecho, então o julgamento sai no dia seguinte, com a chave igual. Vale pra mata-mata, liga e título — em todos o placar de hoje é o que decide.

**Medido.** Sem teste de tela (o caso pede uma queda em copa numa quarta-feira); a regra é uma linha de guarda no topo da função e foi conferida em leitura contra os dois chamadores de `jogarDia`.

## A casa de piscina: a resenha da zona (pedido do dono, 21/09/2026)

**Pedido.** Um cenário novo de festa em casa com piscina: a zona de uma torcida (até 20) está de resenha, 80% em volta da piscina e 20% dentro da casa, com a faixa/bandeira estendida no muro do fundo do deck. Quem é atacado só percebe quando o atacante está na porta; um dos atacados pega a faixa e se tranca num compartimento da casa. Mensagem do feed no molde da do bar ("Chefe, vimos nas redes sociais que a Zona Sul da Cearamor tá fazendo uma resenha deles numa casa com piscina e a faixa deles tá estendida lá, a gente quer dar o bote neles e tomar a faixa."), a gente também pode ser atacado na nossa resenha, e a dose de bar cai 50% pra abrir espaço. "Crie a cena no jogo primeiro em cenas de briga, mas colocando já os atuais bonecos 3d em todas as cenas."

**A foto e a máscara.** `img/cenas/casa_piscina.jpg` (Google Flow, zenital 2000×1116): rua de areia embaixo, lote murado, garagem e casa à esquerda, quintal com dois carros, deck com piscina à direita, portão na quina de baixo/direita. Nesta foto parede e piso são o mesmo creme; a `crista` que serve na sede apagava o deck inteiro ao lado da piscina escura (medido: 22% de chão, quartos fechados). As paredes passaram a ser ditas na mão, em pixel da foto (`CASA(...)` no importador, via `excluir`), e os vãos que a sombra fecharia — porta da frente, porta dos fundos do hall, passagem atrás da casa, boca dessa passagem no deck — são abertos com a opção nova `abrir`. A garagem fica fechada (na foto não tem vão) e os carros são obstáculo. Medido: 33,9% de chão; rua, portão, quintal, deck, sala, hall, dois quartos, banheiro, vestíbulo, depósito e quartinho numa ilha só. O acabamento fino da máscara o dono faz no editor (F2), como avisou.

**A cena (`dados/cenas.js`, `casa-piscina`).** Cinco spawns de quem defende com `guarda:true` (quatro no deck, um na sala) e `espalharBonde:'visitante'` — o bonde único de defesa se reparte 4/4/4/4/4, que é o 80/20 pedido; quando nós atacamos, o rival sem bonde já se reparte por spawn. `gatilho` no portão com `soZona:true`: só a zona acorda a casa — nem a distância (`perto`) nem a linha de visão pela porta, e a faixa fica exposta até a casa acordar (`atualizarFaixa`). `faixas.visitante` no muro do fundo do deck. `faixaAbrigo` no depósito: quem tirou a faixa corre pra lá pelo campo de fluxo e fica (`ramo 'faixa-abrigo'`) — pra tomar a peça o atacante tem de entrar na casa e derrubá-lo. `semBocas:true`: a régua de bocas das ruas puxava o fim da rua (x=40) pra x=420. A cena entra em `CENAS_FAIXA` e em `CHANCE_BANDEIRA` (casa: 50/50).

**O fluxo.** Ataque nosso: cartão `casarival` (texto do dono, ~7,5% das semanas ≈ 4/ano, mesma régua de dia comum do bar, rival hostil da praça, só se ela tem faixa ou bandeira), botões "Dar o bote"/"Deixar quieto" (com Simular, via `ACOES_DE_BRIGA`); `atacar-casa-rival` abre `cena-acao` direto, como a descida da filial, respeitando o limite de um ataque por semana, e fecha por `fecharAtaque` (sem saque — casa de praia não tem caixa; o prêmio é a faixa, por `aplicarFaixa`). Ataque deles: `eventosDoTrimestre` reparte o dado do bar — bar 3/8, casa 3/8 (era bar 3/4) —; `eventoDoTrimestreHoje` marca `ataqueMarcado{alvo:'casa', cena:'casa-piscina', zona}` só se temos faixa ou bandeira, o olheiro avisa, `SOFRIDO.casa` convoca ("Segurar a casa"/"Largar a resenha") e `abrirAtaqueAoBar` bota a nossa zona contra a zona deles. A zona: membro não tem zona marcada, então `acoes.efetivoDaZona` é um quarto do efetivo de pé entre 4 e 20, e `bondeDaZona` sorteia a nossa fatia por hash da semana. `barRivalDeHoje` caiu de 15% pra 7,5% das semanas.

**Na bancada (pedido do dono, 21/09/2026: "coloque a cena no cenas de briga pra eu poder editar a máscara e ver antes de implementar").** Aba "Casa de piscina" em `bancada.js` (20 contra 20, `faixaDefensor:'eles'` — sem save a faixa não existe, como no bar). O `empacotar_cena.py` estava com a lista de scripts de agosto: sem `membros.js`, sem three/GLB/`bonecos3.js`, sem o arranque de `arredores.html` — e sem `<meta charset>`, o que matava a página no primeiro script ("Range out of order" no `[\u0300-\u036f]` do nucleo.js lido como Latin-1). Os três entraram. O 2º escalão mandante saiu de (998,775) pra (1290,790): na bancada os dois spawns dividem o efetivo e a grade dele nascia dentro da zona do portão.

**Bar e casa na mesma frequência (ordem do dono, 21/09/2026).** Um dado só (~15% das semanas, ≈ 8 por ano, o que o bar sozinho era antes) e os dois se revezam: o tipo da vez é o contrário do último que saiu; se não tem alvo hoje (bar hostil na praça / rival com faixa), sai o outro e a vez fica pra próxima. Medido em 3 anos: 11 bares, 12 casas, zero na mesma semana. `E.acoes.ultimaSugestaoTipo` guarda o último.

**A escolta da faixa (pedido do dono, 21/09/2026).** "Na cena da casa, a torcida atacada, se tiver em menor número, vai tentar proteger o portador da faixa, dentro da casa." `escoltaDaFaixa` (combate.js): em cena com `faixaAbrigo`, com a faixa na mão e o lado em menor número, quem não tem inimigo a 110 px vai pro portador e para a 70 px dele (ramo `escolta-faixa`); quem tem inimigo em cima briga como sempre, e o líder do jogador fica de fora. Medido com 10 contra 20: em t=30, 6 dos 9 de pé a menos de 120 px do portador no depósito.

**O protesto só sai da fila se ainda vale (correção do dono, 21/09/2026).** "Meu time ganhou o jogo e mesmo assim gerou a mensagem." O cartão nasce num dia comum com 3 derrotas em 5, fica na fila atrás de outra decisão e aparecia dias depois, em cima da vitória e com a sequência velha. Em `dropar`: sequência que já não é ruim mata a mensagem; dia de jogo nosso devolve pro fim da fila (a bola ainda vai rolar); e o texto é reescrito com a sequência de hoje (`textoDoProtesto`). Medido: com VVEDD a mensagem some da fila e a seguinte sai; com DDEDV sai com o texto atual.

**Uma sugestão de ataque de cada vez (correção do dono, 21/09/2026).** "A mensagem da casa sempre aparece logo após a do bar." Medido em 3 anos simulados (TUF): 7 bares, 14 casas, 2 casas na mesma semana do bar — uma delas no MESMO dia (2027/s6/d2), porque o desvio pro dia comum empurra os dois cartões pro mesmo dia quando a semana tem jogo. As rolagens são independentes (hash FNV, 31 coincidências em 5200 semanas contra 29 esperadas), então não era viés do dado: era falta de espaçamento. Agora `E.acoes.ultimaSugestaoDeAtaque` guarda a semana da última sugestão (bar ou casa) e a outra espera pelo menos 2 semanas. Remedido: 5 bares, 12 casas, zero na mesma semana. A casa segue mais frequente que o bar porque o cartão do bar ainda exige um bar hostil mapeado na praça (filtro que já existia) — a rolagem dos dois é a mesma, 7,5% das semanas.

**Dar o bote sempre abre (ordem do dono, 21/09/2026).** O botão respondia "Não rolou: já saiu bonde pra cima de casa rival esta semana" quando o ataque manual (ou o bote anterior) tinha gastado a vez da semana; o dono não entendeu a mensagem e mandou: dar o bote abre a cena, sempre. O limite semanal do `atacar` saiu do bote, e o bote também não gasta a vez do ataque manual. Só "não rola" se a torcida sumiu do mapa ou a zona não tem 4 de pé. Medido: com `ultimoAtaqueManual` na semana corrente, a resposta abre `cena-acao`.

**A máscara do dono (21/09/2026).** O dono pintou a malha no F2 e mandou a entrada exportada; ela está em `dados/cenas_editadas.js` e manda sobre a do importador. Diferenças: a garagem abriu (a porta da frente da casa dá nela e no quintal), as portas dos cômodos foram redesenhadas, e a passagem atrás da casa saiu — o deck liga à casa só pelo quintal. Remedido com ela: acorda no portão em t=5, faixa na mão em t=9,9, portador chega ao depósito pela sala em t=19,8; 32,3% de chão numa ilha só.
...
## A reunião senta no pátio da sede, e as cinco sedes ganharam foto (decisão do dono, 22/09/2026)

Em vez de gerar uma sétima cena só pra sala da reunião, o dono preferiu gerar logo as fotos dos níveis da sede e fazer a reunião acontecer no pátio de cada uma. As quatro fotos que faltavam (níveis 2, 3, 4 e 5 — o 6 usa a do 5) entraram pelo mesmo importador das outras cenas (`ferramentas/importar_cena_foto.py`, receitas `sede-2` a `sede-5`, semente no pátio e nas salas em volta): mestre `img/cenas/sede_nivel_N.jpg`, saída `sede_N.webp`, máscara e prova `_ref_mascara_sede-N.png`. A `sede-1`, que tinha foto mas não tinha cena, entrou junto: as cinco `sede-N` nascem de um laço em `dados/cenas.js` (`SEDES`), cada uma com o retângulo do pátio medido na foto e, dentro dele, o C quadrado (`cadeirasEmC`: quatro no fundo, quatro em cima, quatro embaixo, todas viradas pra dentro, o presidente sozinho à direita). O retângulo foi conferido contra a máscara, cadeira por cadeira, e ajustado nos níveis 2, 4 e 5 até nenhuma cair em parede — no 4 a coluna do fundo tinha nascido em cima do muro. A cena solta `reuniao`, desenhada, e o pintor `sala` foram embora; ficou o `cadeira` do `cenario`, que agora desenha as cadeiras por cima da foto (`arredores.desenharSobreposicoes`). `sobreFoto` ganhou `semMeio`: a régua de rua (o spawn puxado pro meio da pista) não vale numa cena de dentro. `TO.dados.sedeCenaDoNivel(n)` diz a cena do nível (6 → 5); é por ela que a fase B (`docs/PLANO-REUNIAO-DIRETORIA.md`) vai abrir a reunião na sede do nível atual. Na bancada, as abas "Sede 1" a "Sede 5" mostram cada pátio com a diretoria sentada — é ali que a máscara de cada sede se acerta no F2 (a do nível 4 só abriu o pátio; as salas ficaram escuras). O prompt da sala própria (`img/cenas/PROMPT-REUNIAO.md`) ficou como registro, superado.

## A reunião da diretoria é uma cena no pátio da sede (fase B, pedido do dono, 22/09/2026)

O cartão do dia 5 deixou de abrir a tela da reunião e passou a abrir a cena: o pátio da sede do nível atual (`TO.dados.sedeCenaDoNivel`), a diretoria sentada no C quadrado, o presidente — o jogador — em pé sozinho à direita, e cada pauta num balão de fala em cima do diretor que a traz, como o dono pediu. Os balões são HTML por cima do canvas (`#djBaloes`), ancorados no boneco pela régua cena→canvas da ponte, um por diretor com pauta e um pro presidente (o pedido a um aliado, a nossa jogada nos eixos e as pautas de quem não está na roda). Fechado, o balão é um ponto de exclamação em cima da cabeça — as cadeiras vizinhas ficam a 50 px e um rótulo inteiro cobriria o vizinho; o assunto aparece ao passar o mouse. Aberto, traz o nome de quem fala, a pauta, o quando/contra do bote e os botões. Um aberto por vez: decidido, vira ata cinza e a palavra passa pro próximo; sem lugar em cima abre embaixo; alto demais rola por dentro. Quem está com a palavra gesticula (`falarSentado`, `falarEmPe`) e os outros viram a cabeça pra ele (`olharPara`). Não é briga: sem rival, PM, bomba, placar ou relatório de noite — a faixa da transmissão vira o cabeçalho da reunião com o botão de encerrar, e as teclas de briga não valem (o WASD segue andando). O encerrar passa por `ponte.encerrar`, que na reunião fecha sem XP, moral ou prestígio, e `fecharReuniaoEmCena` fecha a tela, chama `fecharReuniao` no feed (o que não foi decidido fica pra próxima mesa) e marca o cartão. Em tela larga a cena abre em 2× centrada na roda; no celular a ponte já aproxima e a câmera olha pra roda, não pro líder. Os construtores da tela e da cena são os mesmos (`construtoresDaReuniao`); a tela ficou de reserva pra save sem sede ou navegador sem canvas. O que ficou pra fase C está no plano (`docs/PLANO-REUNIAO-DIRETORIA.md`).

## Os convites de festa vão pra reunião da diretoria (pedido do dono, 22/09/2026)

A lista mensal de aniversários das aliadas (o cartão solto do começo do mês, 08/09) deixa de ser cartão e vira pauta da reunião do dia 5 (`feed.pautaFestas`): as aliadas — Aliado ou Irmandade (relação ≥ 20) e as irmãs de clube — cujo aniversário cai da reunião até o dia 4 do mês seguinte, isto é, até a próxima mesa. Dentro do balão cada aliada tem o seu Ir / Não ir (`responderFestaDaPauta`), com o mesmo efeito de sempre — ir custa R$ 2.000 e aproxima (ganho que encolhe a cada festa da mesma torcida no ano), furar afasta e queima prestígio —, e a pauta fecha quando todas tiverem resposta; o que se decidiu sai na ata. A mensagem de convite de cada aliada continua saindo no dia. Convite que ficou sem resposta até a mesa seguinte é festa furada: a lista de uma mesa vai até a véspera da próxima, então na reunião seguinte o que não foi respondido já passou — leva a régua do furar e a pauta fecha, saindo na ata ("0 festas, 3 furadas"). O dono falou em "reunião semanal"; a reunião é a mensal do dia 5 (a régua dele de 22/09), e é nela que os convites entram — festa dos dias 1 a 4 fica com a mesa do mês anterior. O cartão antigo (`aniversarios`) só existe em save antigo e continua respondendo por `responderAniversario`; o efeito de uma resposta é uma função só (`responderFesta`) pros dois.

## Na notícia de véspera, o nosso clube aparece pela posição na força (pedido do dono, 22/09/2026)
...
## O maior rival de nascença vira aliado quando a relação chega lá (correção do dono, 22/09/2026)
